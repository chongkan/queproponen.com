...
global $json_api;
function add_planes_controller($controllers) {
  $controllers[] = 'planes';
  return $controllers;
}
add_filter('json_api_controllers', 'add_planes_controller');

function set_planes_controller_path() {

  return get_template_directory() . "/json-controllers/planes.php";
}
add_filter('json_api_planes_controller_path', 'set_planes_controller_path');
/**
...
function bootstrapwp_scripts_loader() {
    ...
    wp_enqueue_script('bootstrap-js', get_template_directory_uri() . '/assets/js/bootstrap.min.js', array('jquery'), '1.0', false);
   // wp_enqueue_script('demo-js', get_template_directory_uri() . '/assets/js/bootstrapwp.demo.js', array('bootstrap-js'),'1.0',true);
	wp_enqueue_script('main_js', get_template_directory_uri() . '/assets/js/main.js', array('jquery'), '1.0', false);
...
function create_post_type() {
	...
	/* register_post_type( 'likan_partidos',
		 array(
            'labels' => array(
            'name' => __( 'Partidos Politicos' ),
            'singular_name' => __( 'Partido Politico' ),
            'add_new' => __( 'Agregar Partido' ),
            'add_new_item' => __( 'Agregar Nuevo Partido' ),
            'edit' => __( 'Modificar' ),
            'edit_item' => __( 'Modificar Partido Politico' ),
            'new_item' => __( 'Nueva Partido Politico' ),
            'view' => __( 'Ver' ),
            'view_item' => __( 'Ver Partido Politico' ),
            'search_items' => __( 'Buscar Partido Politicos' ),
            'not_found' => __( 'No se encontraron Partido Politicos' ),
            'not_found_in_trash' => __( 'No se encontraron Partido Politicos en la papelera' ),
            'parent' => __( 'Parent Partido Politico' ),
        ),            
        'public' => true,
        'show_ui' => true,
        'show_in_menu' => true,
        ...
        'has_archive' => true,
        'rewrite' => array("slug" => "Partido Politico"),
        'supports' => array('title','editor','thumbnail','page-attributes'),
        'hierarchical' => false,
        'menu_icon' => get_stylesheet_directory_uri() . '/images/costa_rica.png',
        'menu_position' => null
        )
	); */
	register_post_type( 'likan_planes',
		 array(
            'labels' => array(
            'name' => __( 'Propuestas' ),
            'singular_name' => __( 'Propuesta' ),
            'add_new' => __( 'Agregar' ),
            'add_new_item' => __( 'Agregar Nueva Propuestas' ),
            'edit' => __( 'Modificar' ),
            'edit_item' => __( 'Modificar Propuesta' ),
            ...
            'not_found_in_trash' => __( 'No se encontraron Propuestas en la papelera' ),
            'parent' => __( 'Parent Propuesta' ),
        ),            
        'public' => true,
        'show_ui' => true,
        'show_in_menu' => true,
        ...
        'supports' => array('title','editor','thumbnail','page-attributes'),
        'hierarchical' => false,
        'menu_icon' => get_stylesheet_directory_uri() . '/images/the_documents_icon.png',
        'menu_position' => null
        )
	);
...
function add_custom_taxonomies() {
	...
	register_taxonomy('sector', 'likan_planes',  array(
            'labels' => array(
            'name' => __( 'Sectores' ),
            'singular_name' => __( 'Sector' ),
            'add_new' => __( 'Agregar Sector' ),
            'add_new_item' => __( 'Agregar Nuevo Sector' ),
            'edit' => __( 'Modificar' ),
            'edit_item' => __( 'Modificar Sector' ),
            'new_item' => __( 'Nuevo Sector' ),
            'view' => __( 'Ver' ),
            'view_item' => __( 'Ver' ),
            'search_items' => __( 'Buscar' ),
            'not_found' => __( 'No se encontraron Sectores' ),
            'not_found_in_trash' => __( 'No se encontraron Sectores en la papelera' ),
            'parent' => __( 'Parent Sector' ),
        ),            
        'public' => true,
        'show_ui' => true,
        'show_in_menu' => true,
...
/*
add_action( 'add_meta_boxes', 'propuestas_meta_box_add' );  
function propuestas_meta_box_add()  
{  
    add_meta_box( 'partidos_metabox', 'Partido', 'partidos_metabox_callback', 'likan_planes', 'normal', 'high' );  
}
function partidos_metabox_callback($post)  
{  
    global $post;
  	global $wpdb;
    $custom = get_post_custom($post->ID);
    $partidos_c = $custom["partido"];
    $querystr = "
    SELECT wposts.*
    FROM $wpdb->posts wposts
    WHERE wposts.post_type = 'likan_partidos'
    AND wposts.post_title != 'Auto Draft'
    ORDER BY wposts.id DESC
 	";

	 $partidos = $wpdb->get_results($querystr, OBJECT);
	 if( $partidos ) {
	  echo "<select name='partido' id='partido'>";
	  foreach( $partidos as $p ) {
	
		$selected = '';
		if ($partidos_c[0] == $p->ID) $selected ='selected = "selected"';
...
function save_details($post_id){
  ...
  if( isset( $_POST['partido'] ) )  
        update_post_meta( $post_id, 'partido', esc_attr( $_POST['partido'] ) );  
